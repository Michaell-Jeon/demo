...
def generate_static_html_output(output_dir, slicing_columns, model_dir):
    ...
    BUCKET_PATH = "/".join(model_dir.split('/')[3:])

    upload_to_minio(static_html_path, BUCKET_PATH)
    metadata = {
    'outputs' : [{
      'type': 'web-app',
      'storage': 'minio',
      'source': "{}://{}/{}/{}".format("minio",BUCKET_NAME,BUCKET_PATH,static_html_path.split('/')[-1]),
       }]
      }
...
def main():
    ...
    generate_static_html_output(args.output, args.slice_columns, args.model)
